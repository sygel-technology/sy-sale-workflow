...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        for sel in self.filtered(
            lambda a: a.partner_id.receive_confirmation_mails and a.type_id.send_confirmation_mails and a.type_id.so_mail_id and a.state in ('sale', 'done')
        ):
    ...
    def delay_send_so_email(self):
        ...
        self.with_context(avoid_internal_users=True).message_post_with_template(self.type_id.so_mail_id.id)

    @api.multi
    def action_cancel(self):
        super(SaleOrder, self).action_cancel()
        for mail in self.mail_queue_ids.filtered(
            lambda a: a.state not in ['done']
        ):
            mail._change_job_state(
                DONE,
                'Set to done when SO was cancelled'
            )
