    ...
    def button_validate(self):
        for picking in self.filtered(lambda x: x.picking_type_code == 'outgoing'):
            ...
            for move in picking.move_ids.filtered(
               lambda x: x.rule_id.route_id.deposit_operation and
               x.rule_id.route_id.deposit_operation_type ==
               'delivery_deposit'):
                qty_available = move.product_id.with_context(
                    location=locations.id).qty_available
                if qty_available + move.quantity_done > 0:
                    raise UserError(_(
                        'This client in your location does not have enough '
                        'stock. The stock virtual in the location is {}'.
                        format(abs(qty_available))))
