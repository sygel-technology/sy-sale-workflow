...
class SaleOrder(models.Model):
    ...
    def create_monthly_invoices(self, validate=False):      
        sales_to_invoices_ids = self.env['sale.order'].search([
            ('date_order', '>=', (datetime.now() - relativedelta(months=1)).replace(day=1, hour=0, minute=0, second=0)),
            ('date_order', '<', (datetime.now()).replace(day=1, hour=0, minute=0, second=0)),
            ('monthly_invoicing', '=', True), 
            ('invoice_status', '=', 'to invoice')
        ])
        moves = sales_to_invoices_ids.with_context(automatic_cron=True)._create_invoices()
        if validate:
            moves._post()
